...
module.exports = {
	default: function (points, channel){
		...
		var n = {
			name: "The following members had less than " + scannedMembers[0].Points + " points and were not found in-time!",
			value: ""
		};
		var userNameIndex = 0;
		clanMembers.forEach (function (userName){
			if (!clanIgnore.find (j => j.toLowerCase() == userName.toLowerCase()) && !scannedMembers.find (j => j.Name == userName) && userNameIndex < 30){
				userNameIndex ++;
				n.value += (userNameIndex % 8 == 0 ? '\n' : userName + ', ');
			};
		});
	...
	scaninit: function(channel){
		channel.send("Scan initiating... Scanning for players..");
	}
};
